...
第一种：在html中使用`<script>...</script>`标签包含，一般放在在`head`中；

第二种：将js代码放到单独的`.js`文件，然后在html中通过`<script src="..."></script>`引入这个文件；这种方法利于维护和多次调用，推荐！

有时，你会看到`<script type="text/javascript">  ...</script>`声明`type`属性，其实没有必要，因为默认的`type`就是js；

**运行JS**

创建html页面，在其中引入js，然后浏览器加载html文件即可；

本地运行js代码没有问题，但由于浏览器的限制，以`file://`开头的地址无法执行如联网等js代码，所以本地搭建Web服务器很有必要，然后以`http://`开头的地址来正常执行所有js代码。

 **调试**

chrome开发工具，切换到“控制台(Console)”，可以输入js代码，回车执行；

如果还有更高要求，可以使用开发工具中的"源码(Sources)"，掌握断点、单步执行等高级调试技巧。
...
> 计算机：做数学计算的机器，因此计算机程序理所当然可以处理各种数值。但是计算机能处理的不只是数值，还可以出来文本、图像、音频、视频、网页等各种数据，不同数据，需要定义不同的数据类型，js定义了软下几种数据类型：
...
> 计算机使用二进制，所以，有时候石宏十六进制表示整数比较方便，十六进制以`0x`前缀表示。
...
当我们对Number做比较时，可以通过比较运算符得到一个布尔值；实际上，JavaScript允许对任意数据类型做比较：
...
另一个例外是NaN这个特殊的Number与所有其他值都不相等，包括它自己，唯一能判断`NaN`的方法是通过`isNaN()`函数:
